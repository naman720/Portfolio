...
let arts = [
  { image: "textures/pic1.jpg" },
  { image: "textures/pic2.jpg" },
  { image: "textures/pic3.jpg" },
  { image: "textures/pic4.jpg" },
  { image: "textures/pic5.jpg" },
  { image: "textures/pic6.jpg" },
  { image: "textures/pic7.jpg" },
  { image: "textures/pic8.jpg" },
];
...
let artsCameraPos = {
  x: 1.5, // Moving to the opposite x position
  y: 0.45,
  z: 0.498, // Keeping the z to a positive value to be on the opposite side
};
...
function loadIntroText() {
  ...
  loader.load("fonts/unione.json", function (font) {
    ...
    const titleGeo = new TextGeometry("NAMAN RAJ", {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load("fonts/helvatica.json", function (font) {
    ...
    const subTitleGeo = new TextGeometry("Web Designer / Developer / Artist", {
      font: font,
      size: 0.018,
      height: 0,
    });
...
function resetarts() {
  ...
  arts.forEach((art, i) => {
    gsap.to(art.mesh.material, {
      opacity: 0,
      duration: 1,
    });
    gsap.to(art.mesh.position, {
      y: art.y,
      duration: 1,
    });
    gsap.to(art.mesh.scale, {
      x: 0,
      y: 0,
      z: 0,
      duration: 0,
...
function artsMenuListener() {
  // create art planes with textures
  arts.forEach((art, i) => {
    const colIndex = i % 3; // Correct calculation of the column index
    const rowIndex = Math.floor(i / 3); // Correct calculation of the row index
    const geometry = new THREE.PlaneGeometry(0.71, 0.4);
    const material = new THREE.MeshBasicMaterial({
      color: 0xffffff,
      map: new THREE.TextureLoader().load(art.image),
      transparent: true,
      opacity: 0.0,
    });
    const artsPlane = new THREE.Mesh(geometry, material);
    artsPlane.name = "arts";
    artsPlane.position.set(0.3 + colIndex * 0.8, 1 - rowIndex * 0.5, -1.15);
    artsPlane.scale.set(0, 0, 0);
    // mesh & y vars needed for animation
    arts[i].mesh = artsPlane;
    arts[i].y = 1 - rowIndex * 0.5;
    scene.add(artsPlane);
  });

  document.getElementById("arts-menu").addEventListener("click", function (e) {
    e.preventDefault();
    disableOrbitControls();
    resetBookCover();
    gsap.to(camera.position, {
    ...
    // animate & show art items
    arts.forEach((art, i) => {
      art.mesh.scale.set(1, 1, 1);
      gsap.to(art.mesh.material, {
        opacity: 1,
        duration: 1.5,
        delay: 1.5 + i * 0.1,
